...
class PlateOCR:
    ...
    async def recognize(self, plate_image: Image.Image) -> OcrResult | None:
        ...
        # CCT ncnn input is float32 NHWC [64, 128, 3] in the 0-255 range (the uint8->float
        # cast was folded out during conversion). NOTE: the exact ncnn input layout could
        # not be verified on macOS (ncnn extract crashes via MoltenVK) — verify on Linux.
        input_array = np.ascontiguousarray(np.asarray(resized, dtype=np.float32))
        resized.close()
    ...
    def _predict(self, input_array: np.ndarray[Any, Any]) -> list[np.ndarray[Any, Any]]:
        ...
        ex.input("in0", ncnn.Mat(input_array))
        outputs: list[np.ndarray[Any, Any]] = []
        for name in ("out0", "out1"):
            _, mat = ex.extract(name)
            outputs.append(np.squeeze(np.array(mat)))
        return outputs

    @staticmethod
    def _decode_output(outputs: list[np.ndarray[Any, Any]]) -> OcrResult | None:
        # Identify the plate-logits output by shape (10 slots x 37 classes); the other
        # output is the region head (66) which we don't use here.
        plate_logits = None
        for arr in outputs:
            if arr.ndim == 2 and OCR_MAX_SLOTS in arr.shape and len(OCR_ALPHABET) in arr.shape:
                plate_logits = arr if arr.shape[0] == OCR_MAX_SLOTS else arr.T
