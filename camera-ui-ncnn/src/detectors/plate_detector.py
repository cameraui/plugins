...
from defaults import LPD_DETECTOR_MODELS
...
class PlateDetector:
    ...
    async def _do_initialize(self, model_name: str) -> None:
        try:
            ...
            size = LPD_DETECTOR_MODELS.get(model_name, 384)
    ...
    def _predict(self, image: Image.Image) -> np.ndarray[Any, Any]:
        ...
        results = np.squeeze(np.array(out))
        if results.ndim == 2 and results.shape[0] > results.shape[1]:
            results = results.T  # ensure (channels, anchors)
        return results

    @staticmethod
    def _parse_yolov9(results: np.ndarray[Any, Any], threshold: float) -> list[PlateBox]:
        # NCNN LPD models are raw (the end2end NMS tail was stripped during conversion),
        # single class -> row 4 is the confidence. Plugin filters by score; no NMS dedup.
        if results.ndim != 2:
            return []
        ...
        for idx in keep:
            ...
            plates.append({"confidence": conf, "box": (cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2)})
        return plates
