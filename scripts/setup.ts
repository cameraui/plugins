...
let stepNo = 0;
const step = (title: string) => console.log(chalk.bold.cyan(`\n[${++stepNo}] ${title}`));
...
function venvPython(): string {
  return IS_WINDOWS ? join(ROOT, '.venv', 'Scripts', 'python.exe') : join(ROOT, '.venv', 'bin', 'python');
}
...
function setupPython(): void {
  ...
  for (const req of reqFiles) {
    try {
      ...
      failures++;
      warn(`could not install deps for ${basename(dirname(req))} (native/platform dep unavailable?) — continuing`);
    }
  }
...
function main(): void {
  ...
  console.log(chalk.bgGreen.bold('\n DONE '), chalk.green('plugins are set up and ready.'));
}

try {
  main();
} catch (error) {
  console.error(chalk.bgRed.bold('\n FAILED '), chalk.red(error instanceof Error ? error.message : String(error)));
