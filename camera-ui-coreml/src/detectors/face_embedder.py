...
class FaceEmbedder:
    ...
    async def embed(self, image: Image.Image) -> list[float]:
        ...
        out_dict = await asyncio.get_event_loop().run_in_executor(self.executor, self._predict, arr)
        embedding = list(out_dict.values())[0].flatten()
    ...
    def _predict(self, tensor: np.ndarray[Any, Any]) -> Any:
        return self.model.predict({self.input_name: tensor})
