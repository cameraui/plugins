...
class PlateOCR:
    ...
    async def recognize(self, plate_image: Image.Image) -> OcrResult | None:
        ...
        out_dict = await asyncio.get_event_loop().run_in_executor(self.executor, self._predict, input_array)
        return self._decode_output(out_dict)
    ...
    def _predict(self, input_array: np.ndarray[Any, Any]) -> Any:
        return self.model.predict({self.input_name: input_array})

    @staticmethod
    def _decode_output(out_dict: dict[str, Any]) -> OcrResult | None:
        plate_logits = None
